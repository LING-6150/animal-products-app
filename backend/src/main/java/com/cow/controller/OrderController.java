...
public class OrderController {
    ...
    public CommonResult pay(String orderNo, String payType, String cardIds) throws JsonProcessingException {
        if (StringUtils.isNotBlank(orderNo)) {
           Order orderInfo = orderService.selectByOrderNo(orderNo);
           Order order = new Order();
           order.setOrderId(orderInfo.getOrderId());
           order.setPayType(payType);
           order.setUserAccount(orderInfo.getUserAccount());
           ...
           if(orderService.updateById(order)) {
               if(cardIds != null) {
                   ...
                   Integer[] cardIdList = mapper.readValue(cardIds, Integer[].class);

                   if (cardIdList.length > 0) {
                       redisTemplate.delete("cartId" + orderNo);
                       // 遍历 cardIds 删除购物车数据
                       for (Integer cardId : cardIdList) {
                           shoppingCartService.deleteById(cardId);
                       }
                   }
               }
        ...
        return CommonResult.error("订单状态修改失败");
    }
    ...
    public CommonResult cartOrder(String orderNo, String ordersInfo, String cartIds) {
        ...
        if (orders != null) {
            ...
            if (!orderInfo.isEmpty()) {
                ...
                return CommonResult.success("创建订单成功", orderNoInfo);
            }
