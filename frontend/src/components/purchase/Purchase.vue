  ...
  <div class="purchase">
    ...
    <el-card class="purchase-box" shadow="never">
      <div slot="header" style="text-align: left;position: relative">
        ...
        <span>数据列表</span>
        <el-button class="el-button-add" type="primary" @click="addPurchase">添加采购</el-button>
      </div>
      <el-table :data="purchaseData" class="type-table" height="425"  border style="width: 100%;user-select: text">
        ...
        <el-table-column prop="receiptTime" label="收货时间" width="180">
          <template slot-scope="scope" >
            <span v-if="purchaseData[scope.$index].receiptStatus">{{purchaseData[scope.$index].receiptTime}}</span>
            <span v-else></span>
          </template>
        </el-table-column>
    ...
    <!-- 添加采购信息 -->
     <el-dialog title="添加采购信息" class="chang-purchase" :close-on-click-modal="false" :visible.sync="dialogAddVisible" width="800px" center>
      <el-form class="info-form" inline :model="purchaseForm" ref='changeFrom'>
        <el-form-item label="采购编号：" prop="purchaseNo">
          <el-input size="small" readonly v-model="purchaseForm.purchaseNo" placeholder="采购编号"></el-input>
        </el-form-item>
        <el-form-item label="采购数量：" prop="purchaseNumber">
          <el-input size="small" v-model="purchaseForm.purchaseNumber" placeholder="采购数量"></el-input>
        </el-form-item>
        <el-form-item label="选择商品：" prop="productNo">
          <el-select size="small" v-model="purchaseForm.productNo" filterable placeholder="请选择商品">
            <el-option v-for="(item,index) in productInfo" :key="index" :label="item.productName" :value="item.productNo"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="选择供应商：" prop="supplierNo">
          <el-select size="small" v-model="purchaseForm.supplierNo" filterable placeholder="请选择供应商">
            <el-option v-for="(item,index) in supplierInfo" :key="index" :label="item.supplierName" :value="item.supplierNo"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="操作员帐号：" prop="accountNumber">
          <el-input size="small" readonly v-model="purchaseForm.accountNumber" placeholder="操作员帐号"></el-input>
        </el-form-item>
        <el-form-item label="操作员名称：" prop="userName">
          <el-input size="small" readonly v-model="purchaseForm.userName" placeholder="操作员名称"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" style="padding: 12px 80px;" @click="submitPurchase">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods:{
    addPurchase() {
      ...
      this.purchaseForm.purchaseNo = 'P' + Date.now() + '';
      const userInfo = JSON.parse(localStorage.getItem('store'));
      this.purchaseForm.accountNumber = userInfo.user.accountNumber;
      this.purchaseForm.userName = userInfo.user.userName;
      this.purchaseForm.productNo = '';
      this.purchaseForm.supplierNo = '';
    ...
    submitPurchase() {
      ...
      this.purchaseForm.productName = this.productInfo.filter(item=>{return item.productNo===this.purchaseForm.productNo})[0].productName;
      this.purchaseForm.supplierName = this.supplierInfo.filter(item=>{return item.supplierNo===this.purchaseForm.supplierNo})[0].supplierName;


      this.$http.post('/purchase/addPurchase', this.$qs.stringify(this.purchaseForm)).then((rep)=>{
        if(rep.data.code===200){
          this.dialogAddVisible = false;
          this.$notify({title: '成功', message: '采购信息提交成功', type: 'success'});
          this.reqInfo();
        }
      }).catch((err)=>{this.$msg.error(err)})
    },
