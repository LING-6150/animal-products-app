  ...
  <div class="product-specs">
    ...
    <el-card class="product-specs-box" shadow="never">
      <div slot="header" style="text-align: left;position: relative">
        ...
        <span>数据列表</span>
        <el-button class="el-button-add" @click="addBrand" type="primary">添加</el-button>
      </div>
    ...
    <el-dialog title="编辑商品类型信息" class="chang-specs" :close-on-click-modal="false" top="30vh" width="500px" :visible.sync="dialogFormVisible" center>
  ...
  export default {
    ...
    methods:{
      ...
      deleteBrand(index, rows){
        ...
        this.$confirm('此操作将永久删除该商品类型信息, 是否继续?', '提示', {
          ...
          this.$http.post('/specs/delete?specsId='+this.productSpecs[index].specsId).then((rep)=>{
            ...
            if(rep.data.code === 200){
              ...
              this.reqInfo();
              this.$notify({title: '成功', message: '商品类型('+name+')\t移除成功', type: 'success'});
            }else{
              this.$notify({title: '错误', message: '商品类型('+name+')\t移除失败', type: 'error'});
            }
          }).catch((err)=>{
            loading.close();
            this.$msg.error(err);
          ...
          if (valid) {
            if(this.insert){
                  ...
                  if(rep.data.data){
                    ...
                    this.$http.post('/specs/add',this.$qs.stringify(this.alterForm, { skipNulls: true })).then((rep)=>{
                      ...
                      if(rep.data.code===200){
                        ...
                        this.reqInfo();
                        this.$notify({title: '成功', message: '商品类型('+this.alterForm.specsName+')\t添加成功', type: 'success'});
                      }else{
                        this.$notify({title: '错误', message: '商品类型('+this.alterForm.specsName+')\t添加失败', type: 'error'});
                      }
              ...
              this.$http.post('/specs/existsSpecsName?specsId='+this.alterForm.specsId+'&productType='+this.alterForm.productType+'&specsName='+this.alterForm.specsName).then((rep)=> {
                if (rep.data.code === 200) {
                  if (rep.data.data) {
                    this.$alert('商品类型('+this.alterForm.productType + ')\t已存在，无法完成修改操作', '警告', {
                      confirmButtonText: '确定',
                      type: "warning"
                    });
                  } else {
                    ...
                    this.$http.post('/specs/update', this.$qs.stringify(this.alterForm, { skipNulls: true })).then((rep) => {
                      ...
                      if (rep.data.code === 200) {
                        this.dialogFormVisible = false;
                        this.reqInfo()
                        this.$notify({title: '成功', message: '商品类型更新成功', type: 'success'});
                      } else {
                        this.$notify({title: '错误', message: '商品类型更新失败', type: 'error'});
                      }
                    }).catch((err) => {
                      loading.close();
                      this.$msg.error(err)
