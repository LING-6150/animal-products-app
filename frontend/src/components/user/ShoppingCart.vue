<template>
  <div class="shopping-cart">
    <el-button v-if="this.tableData && tableData.length!==0" class="clear-cart" type="primary" icon="el-icon-delete-solid" title="清空Shopping Cart" @click="clearCart" circle></el-button>
      ...
      <div class="nav-bar">
        <div class="container">
          ...
          <div style="display: inline-block;position:absolute;top: 15px">
            <h2 style="margin-left: 20px; font-size: 20px;">我的Shopping Cart</h2>
            <div class="tip">
              <span></span>
            </div>
          </div>
...
import { set } from 'vue';
    ...
    async surePay() {
      ...
      for(let i=0;i<this.orderNoList.length;i++) {
        await this.$http.post('/order/pay',this.$qs.stringify({orderNo: this.orderNoList[i], payType: this.paymentType, cardIds: this.cardIds },{skipNulls: true})).then((rep)=>{}).catch((err)=>{loading.close();this.$msg.error(err);})
      }
    ...
    paymentTypeChange (val) {
      ...
      console.log("val => ", val)
        ...
        createOrder(){
          ...
          this.$http.post('/order/cartOrder',this.$qs.stringify(send)).then((rep)=>{
            ...
            if(rep.data.code===200){
              let orderNoList = rep.data.data.split(',');

              this.payFormVisible = true;
              this.totalPrice = this.baseInfo.payMoney;
              this.currentOrderNo = orderNo;
              this.paymentType = '微信'
              this.orderNoList = orderNoList;
              this.cardIds = send.cartIds;
            }
        ...
        deleteProduct(index){
          this.$confirm('此商品将从Shopping Cart中移除?', '提示', {
        ...
        clearCart(){
          this.$confirm('是否清空Shopping Cart？', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            ...
            this.$http.post('/shoppingCart/deleteByUser?accountNumber='+this.$store.state.user['accountNumber']).then((rep)=>{
              ...
              if(rep.data.code===200){
                this.reqInfo();
                this.$message({type: 'success', message: 'Shopping Cart已清空!'});
              }
            }).catch((err)=>{loading.close();this.$msg.error(err)})
          }).catch(() => {});
        },
